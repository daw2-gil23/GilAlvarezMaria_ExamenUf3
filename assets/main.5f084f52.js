  ...
  template: `
    <div class="col-6" id="registrarPedido">
        ...
        <form id="FormCervezas" class="needs-validation w-50 pt-2 ps-2 FormCervezas" novalidate>
            ...
            <button id="enviar" type="submit" class="btn btn-success w-100 enviarPedido">A\xF1adir Pedido</button>
        </form>
    </div>
    ...
    main2.addEventListener("click", (event) => {
      if (event.target.classList.contains("enviarPedido")) {
        ...
        if (formPedido.checkValidity()) {
          ...
          pedidos.forEach((pedido2) => {
            sumaPrecio = sumaPrecio + pedido2.precioTotalPedido;
          });
          const pedidoNuevo = {
            id: idNuevo,
            nombre: inputNombre,
      ...
      if (event.target.classList.contains("eliminar")) {
        ...
        alert("Est\xE1s borrando el pedido con id: " + pedidoId);
        const trId = document.getElementById(pedidoId);
        console.log(trId);
        trId.classList.add("fila-oculta");
        const posicion = pedidos.findIndex((pedido2) => pedido2.id == pedidoId);
      ...
      if (event.target.classList.contains("editar")) {
        ...
        var html = ``;
        pedidos.forEach((pedido2) => {
          if (pedido2.id == pedidoID) {
            const posicion = cervezas.findIndex((cerveza) => cerveza.nombre == pedido2.cerveza);
            cervezas.forEach((cerveza) => {
            ...
            document.getElementById("cantidad").value = pedido2.cantidad;
          }
        });
        const select = document.querySelector("#cervezas");
        document.querySelector("#EditarPedido");
        select.innerHTML = html;
      }
      if (event.target.classList.contains("actualizar")) {
        ...
        const posicionPedido = pedidos.findIndex((pedido2) => pedido2.id == pedidoID);
...
document.querySelector("#pedidos").innerHTML = pedido$1.template;
pedido$1.script();
